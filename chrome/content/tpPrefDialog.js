...
var TrackPackage_prefs =
{
 ...
 tpSaveSettings: function()
 {
  ...
  for (index = 0; index < urlCount; index++)
  {
   ...
   if (typeof urlFront === 'undefined' || urlFront === null)
    urlFront    = nodes.item(1).getAttribute('value');
   var urlBack  = nodes.item(2).value;
   if (typeof urlBack === 'undefined' || urlBack === null)
  ...
  {
   if (typeof regexCarrierArray[i] === 'undefined' || regexCarrierArray[i] === null || regexCarrierArray[i] === '')
    continue;
   var foundMatch = false;
   for (var j = 0; j < urlCount; j++)
   {
    if (typeof urlCarrierArray[j] === 'undefined' || urlCarrierArray[j] === null || urlCarrierArray[j] === '')
     continue;
